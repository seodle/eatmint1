...
max.window.size <- 5 #size of the window = number of collaborative acts per window : the size of the window is shorter when another emotion is encountered before max.window.size is reached or the end of the collaborative acts
```
...
#compute the real-time use for every collaborative act
...
write.xlsx(emotions.results, "results15.xlsx") 

=======

```{r}
write.xlsx(emotions.results, "results5.xlsx")
>>>>>>> 15_acts_problem
```
